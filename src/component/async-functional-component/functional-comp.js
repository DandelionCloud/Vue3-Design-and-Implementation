 ...
 * 2. 函数式组件的实现（与有状态组件类似）
 *  - 在 patch 函数中增加对于函数式组件的判断 ===> typeof vnode.type === 'function'
 *  - 挂载方式可复用 mountComponent 方法 ===> 增加是否是函数式组件的判断（同上）
 */
...
function createRenderer(options) {
    ...
    function patch(n1, n2, container, anchor) {
        ...
        // 有状态组件（vnode.type 的值是组件选项对象）
        // 函数式组件（vnode.type 的值是函数）
    ...
    function mountComponent(vnode, container, anchor) {
        // 检查是否为函数式组件
        const isFunctionalComp = typeof vnode.type === 'function'

        const componentOptions = vnode.type
        // 如果是函数式组件，则将 vnode.type 作为渲染函数，vnode.type.props 作为 props 选项定义
        if(isFunctionalComp){
            componentOptions = {
                render: vnode.type,
