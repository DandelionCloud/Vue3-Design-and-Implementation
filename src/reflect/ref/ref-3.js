 ...
 * 2. 修改响应式数据时，不会触发重新渲染
 * 
 * 自动脱 ref：指的是属性的访问行为，即读取一个 ref 属性时，直接将 ref 对应的 value 属性值返回。
 */
...
/**
 * 封装一个 toRef 函数：
 * 1. 将响应式对象的属性转换为含有一个访问器属性 value 的对象
 * 2. 概念统一：将通过 toRef 或 toRefs 转换后得到的结果视为真正的 ref 数据
 * 3. 解决 toRef 返回的数据只读：允许设置值
 */
...
/**
 * 封装一个 toRefs 函数，将响应式对象的所有属性进行转换
 * 1. 使用 for...in 循环遍历对象
 * 2. 逐个调用 toRef 函数完成属性到含有一个访问器属性 value 的对象的转换
 */
function toRefs(obj) {
    const res = {}
...
/////////////////////////////////////////////// 自动脱 ref //////////////////////////////////////////////////
/**
 * 属性的访问行为：如果读取的属性是一个 ref，则直接将该 ref 对应的 value 属性值返回
 * 如 newObj2.foo 是一个 ref，访问 newObj2.foo 即访问 newObj2.foo.value
 */
/**
 * 封装一个 proxyRef 函数，实现自动脱 ref 和自动为 ref 设置值
 * 1. 读取属性，触发 get 拦截函数
 *    - 当读取的属性是一个 ref 时，则直接返回该 ref 的 value 属性值
 * 2. 设置属性，触发 set 拦截函数
 *    - 如果设置的属性是一个 ref，则间接设置该 ref 的 value 属性的值
 */
...
const newObj3 = proxyRefs({ ...toRefs(obj) })

effect(() => {
    console.log('proxy-', newObj3.foo)
})
newObj3.foo = 4