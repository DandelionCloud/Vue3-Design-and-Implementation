 ...
 * 
 * 当前实现的是一个 shallowReactive
 * 
 * shallowReactive: 浅响应，即只有对象的第一层属性是响应的
 */
...
setTimeout(() => {
  // obj.foo.bar 不是相应的，不能触发副作用函数重新执行
  obj.foo.bar = 2
 ...
 * reset obj.foo
 * 
 * 浅响应：只有对象的第一层属性是响应的，第二层及更深层次的属性不是相应的
 * 
 * 结果：
 * 1. 设置 obj.foo.bar 的值时，obj.foo.bar 不是响应式的，不能没有触发副作用函数重新执行
 * 2. 设置 obj.foo 的值，obj.foo 是响应式的，能够触发副作用函数重新执行
 */
