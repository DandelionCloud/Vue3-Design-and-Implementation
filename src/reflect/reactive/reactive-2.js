 ...
 * 思路：屏蔽原型上 set 中触发响应
 * 
 * 特点：设置详情是数据 child.bar 的值时，不论什么情况下 receiver 都是 child
 * 
 * 解决：在 set 拦截函数中，只有当 receiver 是 target 的代理对象是，才能触发副作用函数重新执行
 * 
 * 实现：
 * 1. get 拦截增加功能：代理对象通过 raw 属性获取原始数据
 * 2. set 拦截函数判断 receiver 是不是 target 的代理对象：if(target === receiver.raw)
 */
...
setTimeout(() => {
  ...
  console.log('代理对象 child 的原始对象是 obj', child.raw === obj)
  console.log('代理对象 parent 的原始对象是 proto', parent.raw === proto)
}, 2000)
 ...
 * --- 2秒后 ---
 * child.bar 2
 * 代理对象 child 的原始对象是 obj true
 * 代理对象 parent 的原始对象是 proto true
 * 
 * 前置知识：设置 child.bar 的值，所以不论在什么情况下，receiver 都是 child
 * 
 * 解释：
 * 1. 响应被触发两次，则屏蔽一次 ===> 屏蔽 parent.bar 触发的那一次
 * 2. 在 set 拦截函数中触发响应 ===> 在 set 拦截函数中区分这两次响应
 * 结论：在 set 拦截函数中，只有当 receiver 是 target 的代理对象时，才触发响应
 * 
 * 实现过程：
 * 1. get 拦截增加功能：代理对象通过 raw 属性获取原始数据
 * 2. set 拦截函数判断 receiver 是不是 target 的代理对象：if(target === receiver.raw)
