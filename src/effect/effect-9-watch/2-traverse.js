/**
 * 【watch】（effect + options.scheduler）
 * 本质：观测一个响应式数据，当数据发生变化时通知并执行相应的回调函数
 * 
 * 注意：回调函数在 scheduler 函数中执行，所以第一次回调函数并不会执行
 * 
 * 问题：硬编码读取 source.foo
 * 解决：封装通用的读取操作 traverse 函数
 * 1. traverse 函数：递归的读取对象上的每一个属性
 * 2. 读取操作：所有属性都与副作用函数建立了联系
 * 3. 任一属性值变化，都可以触发调度器 scheduler 执行
 */
 ...
 * @returns
 * 注释：
 * 1. 副作用函数中读取一个属性值，就会建立该副作用函数和这个对象及其属性的联系
 * 2. 属性值变化时，可以从联系中找到关联的副作用函数，及其 options.scheduler
 */
